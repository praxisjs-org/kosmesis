...
Appears under "Features" header, `registry` subheader:

```
feat(registry): add data-table component
```

Appears under "Bug Fixes" header, `cli` subheader, with a link to issue #12:

```
fix(cli): resolve registry dependencies in the wrong order
...
```
perf(cli): cache resolved registry tree across component adds

BREAKING CHANGE: `kosmesis add` no longer accepts a bare registry URL as its first argument.
```

The following commit and commit `667ecc1` do not appear in the changelog if they are under the same release. If not, the revert commit appears under the "Reverts" header.

```
revert: feat(registry): add data-table component
...
`@kosmesis/` prefix), or one of the private workspaces:

`cli`, `registry`, `docs`, `storybook`
