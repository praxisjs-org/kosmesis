...
const pulse = keyframes("kosmesis-tool-call-pulse", { "0%, 100%": { opacity: "1" }, "50%": { opacity: "0.5" } });

class ToolCallStyles extends Stylesheet {
  $root = this.css({
    borderRadius: "0.5rem",
...
export class ToolCall extends StatefulComponent {
  @Styled(ToolCallStyles) $s!: ToolCallStyles;
  ...
  render() {
    return (
      <div data-slot="tool-call" data-status={() => this.status} class={cx(this.$s.$root, this.class)}>
