...
class SortableStyles extends Stylesheet {
  $root = this.css({ position: "relative", display: "flex", flexDirection: "column", gap: "0.5rem" });
  ...
  // Assigned imperatively (`ghost.className = this.$s.$ghost`) — the placeholder is a plain `div`
  // created outside JSX so it can stay a child of `container` while the dragged item is lifted out.
  $ghost = this.css({
    borderRadius: "0.375rem",
    border: `2px dashed color-mix(in oklch, ${t.mutedForeground} 40%, transparent)`,
    backgroundColor: `color-mix(in oklch, ${t.muted} 30%, transparent)`,
  });

  $handle = this.css({ cursor: "grab", touchAction: "none", color: t.mutedForeground }).on("&:active", {
    cursor: "grabbing",
  });
}
...
export class Sortable extends StatefulComponent {
  @Styled(SortableStyles) $s!: SortableStyles;
...
export class SortableItem extends StatelessComponent<SortableItemProps> {
  ...
  render() {
    ...
    return (
      <div data-slot="sortable-item" data-value={value} class={cx(this.$s.$item, cls)}>
        {children}
      </div>
    );
  }
...
export class SortableHandle extends StatelessComponent<SortableHandleProps> {
  ...
  render() {
    ...
    return (
      <span data-slot="sortable-handle" aria-hidden class={cx(this.$s.$handle, cls)}>
        ⠿
      </span>
    );
  }
